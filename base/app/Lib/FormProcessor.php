...
class FormProcessor
{
    public function generatorValidation()
    {
        $validation['rules'] = [
            ...
            'form_generator.form_type.*'=>'required|in:text,select,radio,textarea,checkbox,file,email,number,date',
            'form_generator.placeholder' => 'nullable'
        ];
        $validation['messages'] = [
        ...
        if ($forms) {
            for ($i=0; $i < count($forms['form_label']); $i++) {
                ...
                $formData[$label] = [
                    ...
                    'placeholder' => $forms['placeholder'][$i],
                ];
            }
        }
        if ($isUpdate) {
