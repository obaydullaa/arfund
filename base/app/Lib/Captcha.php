...
class Captcha{
    ...
    */
    public static function reCaptcha(){
        $reCaptcha = Plugin::where('act', 'google-recaptcha2')->where('status', Status::ENABLE)->first();
    ...
    */
    public static function customCaptcha($width = '100%', $height = 46, $bgColor = '#003'){
        ...
        $captcha = Plugin::where('act', 'custom-captcha')->where('status', Status::ENABLE)->first();
        if (!$captcha) {
            return 0;
        }
        ...
        foreach ($char as $value) {
            $ret .= '<span style="    float:left;     -webkit-transform: rotate(' . rand(-50, 50) . 'deg);">' . $value . '</span>';
        }
        $ret .= '</div>';
    ...
    */
    public static function verifyGoogleCaptcha(){
        $pass = true;
        $googleCaptcha = Plugin::where('act', 'google-recaptcha2')->where('status', Status::ENABLE)->first();
        if ($googleCaptcha) {
    ...
    */
    public static function verifyCustomCaptcha(){
        $pass = true;
        $customCaptcha = Plugin::where('act', 'custom-captcha')->where('status', Status::ENABLE)->first();
        if ($customCaptcha) {
