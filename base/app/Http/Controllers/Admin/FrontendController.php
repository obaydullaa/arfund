...
class FrontendController extends Controller
{
    ...
    public function frontendContent(Request $request, $key)
    {
        ...
        foreach ($valInputs as $keyName => $input) {
            ...
            if(str_contains($keyName, '_editor'))
            {
                $newpurifierInput = summernoteContent($input, $request->id ? $content->data_values->$keyName : false);
                $newpurifierInput = str_replace('<br />', '<br>', $newpurifierInput);
                $newpurifierInput = str_replace('<br \\/>', '<br>', $newpurifierInput);
                $newpurifierInput = str_replace('<\/', '</', $newpurifierInput);
                $newpurifierInput = str_replace('\\r\\n', '', $newpurifierInput);
                $inputContentValue[$keyName] = $newpurifierInput;
            }else{
