...
class ManageUsersController extends Controller
{
    ...
    public function allUsers()
    {
        $pageTitle = 'All Users';
        $users = $this->userData();
        if(request()->search || request()->search_date || request()->date_range)
        {
            $users = $this->userSearch(request()->except('_token'));
    ...
    protected function userData($scope = null){
        ...
        return $users->searchable(['username','email'])->orderBy('id','desc')->paginate(getPaginate(10));
    }
    ...
    {
        $user = User::findOrFail($id);
        $pageTitle = 'Login History - @' . $user->username;
        $loginLogs = UserLogin::where('user_id',$id)->orderBy('id','desc')->with('user')->paginate(getPaginate());
        return view('admin.users.logins', compact('pageTitle', 'loginLogs'));
    }
    ...
    public function sendNotificationAll(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'send_via'                     => 'required|in:all,sms,email',

        ], [
            'number_of_days.required_if'               => "Number of days field is required",
            'number_of_top_deposited_user.required_if' => "Number of top deposited user field is required",
            'send_via.required'                       => "Notification sending media is required",
            'send_via.in'                       => "Notification sending media can be all media, email and sms",
        ]);
        ...
        $media = $sendVia == 'all' ? ['sms', 'email'] : ($sendVia == 'sms' ? ['sms'] : ($sendVia == 'email' ? ['email'] : []));
    ...
    public function userSearch($data)
    {
        ...
        {
            $search = $data['search'];
            $users = $users->whereAny(['username','email','mobile'],'LIKE',"%$search%");
        }

        if(@$data['search_date'] != '' || @$data['search_date'] != null)
        {
            if($data['search_date'] == 'today')
            {
                $users->whereDate('created_at', Carbon::today());
            }elseif($data['search_date'] == 'yesterday')
            {
                $users->whereDate('created_at', Carbon::yesterday());
            }elseif($data['search_date'] == 'last_week')
            {
                $users->whereBetween('created_at', [Carbon::now()->startOfWeek()->subWeek(), Carbon::now()->startOfWeek()]);
            }elseif($data['search_date'] == 'this_week')
            {
                $users->whereBetween('created_at', [Carbon::now()->startOfWeek(), Carbon::now()]);
            }
            elseif($data['search_date'] == 'this_month')
            {
                $users->whereYear('created_at', now()->year)->whereMonth('created_at', now()->month);
            }
            elseif($data['search_date'] == 'last_month')
            {
                $users->whereYear('created_at', now()->subMonth()->year)->whereMonth('created_at', now()->subMonth()->month);
            }elseif($data['search_date'] == 'custom_date')
            {
                $date = explode(' - ', $data['date_range']);

                if(SiteSetting::first()->date_format  === 'mm-dd-yyyy')
                {
                    $startDate = \Carbon\Carbon::createFromFormat('m-d-Y', $date[0])->format('Y-m-d');
                    $endDate = @$date[1] ? \Carbon\Carbon::createFromFormat('m-d-Y', $date[1])->format('Y-m-d') : $startDate;
                }else{
        ...
        $users = $users->orderBy('id','desc')->paginate(getPaginate(10));
